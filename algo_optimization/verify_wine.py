# -*- coding: utf-8 -*-
"""
Verify wine_value_reversal algorithm for sh512690
Expected: 80 signals, 85.0% win rate, +0.37% return, +2.37% max return
"""
...
def main():
    ...
    assert 'wine_value_reversal' in ALGORITHM_MAP, "wine_value_reversal not in ALGORITHM_MAP!"
    algo = get_algorithm('wine_value_reversal')
    assert algo.name == 'wine_value_reversal', f"Wrong name: {algo.name}"
    logger.info("PASS: wine_value_reversal registered in ALGORITHM_MAP")

    # 2. Verify etf_config
    etf = get_etf_by_code(ETF_CODE)
    assert etf is not None, f"{ETF_CODE} not found in ETF_POOL!"
    assert etf.algorithm == 'wine_value_reversal', f"Wrong algorithm: {etf.algorithm}"
    ...
    expected = {'signals': 80, 'win_rate': 85.0, 'avg_max_return': 2.37}
    print(f"\n{'='*70}")
    print(f"  Wine Value Reversal Verification - {ETF_NAME} ({ETF_CODE})")
    print(f"{'='*70}")
    print(f"  Expected: {expected}")
    print(f"  Actual:   signals={r['signals']}, win_rate={r['win_rate']}%, "
          f"avg_max_return={r['avg_max_return']:+.2f}%")

    if r['signals'] > 0 and r['win_rate'] >= 83:
        print(f"\n  RESULT: PASS (win_rate {r['win_rate']}% >= 83%)")
    else:
