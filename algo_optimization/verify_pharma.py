# -*- coding: utf-8 -*-
"""
Verify pharma_reversal algorithm for sz159992
Expected: 23 signals, 91.3% win rate, +0.61% return, +2.53% max return
"""
...
def main():
    ...
    assert 'pharma_reversal' in ALGORITHM_MAP, "pharma_reversal not in ALGORITHM_MAP!"
    algo = get_algorithm('pharma_reversal')
    assert algo.name == 'pharma_reversal', f"Wrong name: {algo.name}"
    logger.info("PASS: pharma_reversal registered in ALGORITHM_MAP")

    # 2. Verify etf_config
    etf = get_etf_by_code(ETF_CODE)
    assert etf is not None, f"{ETF_CODE} not found in ETF_POOL!"
    assert etf.algorithm == 'pharma_reversal', f"Wrong algorithm: {etf.algorithm}"
    ...
    expected = {'signals': 23, 'win_rate': 91.3, 'avg_max_return': 2.53}
    print(f"\n{'='*70}")
    print(f"  Pharma Reversal Verification - {ETF_NAME} ({ETF_CODE})")
    print(f"{'='*70}")
    print(f"  Expected: {expected}")
    print(f"  Actual:   signals={r['signals']}, win_rate={r['win_rate']}%, "
          f"avg_max_return={r['avg_max_return']:+.2f}%")

    if r['signals'] > 0 and r['win_rate'] >= 88:
        print(f"\n  RESULT: PASS (win_rate {r['win_rate']}% >= 88%)")
    else:
